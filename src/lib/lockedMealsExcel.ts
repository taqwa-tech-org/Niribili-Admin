...
export async function downloadLockedMealsExcel(params: {
  ...
  wb.creator = "Niribili Bachelor Home";
  wb.created = new Date();
  ...
  title.value = "নিরিবিলি ব্যাচেলর হোম  •  Niribili Bachelor Home";
  title.font = {
    name: FONT_NAME,
    size: 20,
    bold: true,
  ...
  for (const building of buildings) {
    ...
    // Staff rows — placeholder rows for office staff meals (Cooker + Office).
    // Cells are intentionally left blank so the admin can write the actual
    // counts by hand on the printed report. These rows are NOT counted in
    // the Sub Total / Grand Total — those stay based on flat-resident orders.
    const staffLabels = ["Cooker", "Office"];
    staffLabels.forEach((staffName) => {
      const cells: (string | number)[] = ["", staffName, "", "", "", ""];
      cells.forEach((value, c) => {
        const cell = ws.getCell(row, c + 1);
        cell.value = value;
        cell.font = {
          ...
          size: 11,
          color: { argb: C.text },
          bold: c === 1, // Staff label bold...
          italic: c === 1, // ...and italic, so it reads as non-flat
        };
        cell.alignment = {
          horizontal: c === 0 ? "left" : "center",
          vertical: "middle",
  ...
  footer.value = `Generated by Niribili Bachelor Home Admin  •  ${new Date().toLocaleString()}`;
  footer.font = {
    name: FONT_NAME,
    size: 9,
    italic: true,
